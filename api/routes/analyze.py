...
- 대화의 양이 많습니다. 분석 결과(`analysis_items`, `compatibility_issues`)는 가장 핵심적이고 치명적인 원인 3~4개만 요약해서 간략히 작성하세요. 길이가 길어지면 안 됩니다.
- [절대 규칙 0] 절대 분석결과에 target_name을 포함시키지 마시오. target_name은 인스타 아이디,한글,숫자 등 여러 형태로 포함되어있으니 이를 정확히 인지후 포함시키지 마세요.
- [절대 규칙 1] `evidence` 속성에는 **반드시 제공된 [분석 대상 대화] 안에 실제로 존재하는 문장만** 그대로 인용해서 입력하세요. 대화록에 없는 내용을 지어내거나 쓰면 안 됩니다.
- [절대 규칙 2] 모든 분석 내용(`description`, `issue`, `detail`, `final_verdict` 등)은 철저하게 **제공된 대화 내용에서 일어난 사실에만** 기반해야 합니다. 분석 대상이 하지도 않은 말이나 행동을 임의로 상상하여 추가하지 마세요.
- [절대 규칙 3] 모든 'analysis_items' 의 추출하는 순위는 count가 높은 순으로 정렬하세요.
{user_type_context}
