...
아래 [공공데이터 기반 위험 행동 분류 체계]를 기준으로 분석 대상자의 대화 패턴을 분석하라.
...
[규칙 1-A] 익명성 — 최우선 규칙, 예외 없음
- JSON의 모든 텍스트 필드(behavior, description, detail, evidence, comment, issue 등)에서
  대화에 등장하는 실제 인물 이름을 절대 포함시키지 마라. 본인·상대방 이름 모두 해당.
- evidence에 대화를 인용할 때 화자 이름 부분은 반드시 아래와 같이 대체하라.
  분석 대상자('{target_name}')의 이름 = "상대방"
  그 외 화자(본인)의 이름 = "본인"
  예) 분석 대상자가 '채윤'일 때 "채윤 : 지금 어디야?" → "상대방 : 지금 어디야?"
  예) 분석 대상자가 '채윤'일 때 "태현 : 군산 가야해서" → "본인 : 군산 가야해서"
- description, detail, comment에서도 이름 대신 '본인', '상대방'으로만 지칭하라.
- 대화 내용(evidence) 속에 포함된 모든 실명과 호칭을 반드시 찾아내어 마스킹하라.
- 본인 :, 상대방 : 라벨링을 철저히 준수할 것.
- 화자명(Speaker)뿐만 아니라, 대화 본문(내용) 텍스트 안에 포함된 이름과 호칭(~~아, ~~야 등)도 단 한 글자도 빠짐없이 찾아내어 반드시 '본인' 또는 '상대방'으로 마스킹하라.
- 조사 유지: 이름 뒤에 붙는 조사나 호칭(~아, ~야, ~은, ~는)은 유지하되, 이름 글자만 '본인/상대방'으로 바꿀 것. (예: "철수야" → "상대방아")
- 최종 출력 전, 결과물에 사용자나 상대방의 실제 이름이 단 한 글자라도 포함되어 있다면 그것은 실패한 데이터로 간주한다.
- 이름이 단 한 글자라도 포함된 출력은 어떤 이유로도 허용되지 않는다.
...
- 이름은 반드시 '본인 :' / '상대방 :'으로 대체할 것 (규칙 1-A 참고).
- 인용하는 대화 내역이 여러 줄일 경우, 줄바꿈(\n) 기호와 들여쓰기를 사용하여 깔끔하게 포맷팅하라. (예: "  상대방 : 지금 어디야?\n  본인 : 집이야")
...
{{
  ...
  "analysis_items": [
    {{
      ...
      "count": 0,
      "likability_score": 0,
      "description": "요약 (50자 이내, 이름 금지)",
      "evidence": "실제 대화 인용. 이름은 본인:/상대방: 으로 대체, 여러 줄일 경우 줄바꿈(\\n)과 들여쓰기를 사용하여 깔끔하게 포맷팅할 것. (없으면 null)"
    }}
  ],
  "compatibility_issues": [
    {{ "issue": "충돌 원인 (이름 금지)", "severity": "높음/중간/낮음", "detail": "충돌 발생 원인에 대한 설명(100자 이내, 이름 금지)을 작성한 후, 줄바꿈(\\n)을 하고 관련된 실제 대화 내역을 2개 이내로 인용하여 포함할 것. 대화 인용 시 이름은 본인:/상대방: 으로 대체하고 시간은 삭제하며 들여쓰기로 깔끔하게 포맷팅할 것" }}
  ],
  "danger_level": "안전 / 주의 / 경고 / 위험",
  "danger_comment": "데이트폭력 위험 신호 관련 한줄 코멘트 (위험 신호 없으면 null, 이름 금지)",
